...
namespace AmbarPPC
{
	...
	public class DepoSecimOzel : System.Windows.Forms.Form
	{
		...
		private void Doldur()
		{
			if(Rapor)
			{
                Utility.Engine.sql = "SELECT distinct DepoKodu  FROM  depo_tanim depo (NOLOCK) where  Level_='0' order by DepoKodu";
			}
			else 
			{
                Utility.Engine.sql = "SELECT distinct DepoKodu  FROM  depo_tanim depo (nolock) where  (Level_ = '0') and DepoKodu not in (select AD from haric_depo) order by DepoKodu";
				if(Iade)
		...
		private void DetayDoldur(TreeNode tn)
		{
			...
			if(Rapor)
			{
                Utility.Engine.sql = string.Format("SELECT distinct DepoKodu   FROM  depo_tanim  (NOLOCK) where (UstDepoKodu=N'{0}')   order by DepoKodu", tn.Tag.ToString());
			}
			else 
			{
                Utility.Engine.sql = string.Format("SELECT distinct DepoKodu   FROM  depo_tanim  (NOLOCK) where (UstDepoKodu=N'{0}')  and (DepoKodu not in (Select AD from haric_depo))  order by DepoKodu", tn.Tag.ToString());
				if(Iade)
					Utility.Engine.sql = Utility.Engine.sql.Replace("not","");
			}
		...
		private void button_TAMAM_Click(object sender, System.EventArgs e)
		{
			...
			{
                Utility.Engine.sql = string.Format("SELECT DISTINCT   DepoKodu,Sevk   FROM  depo_tanim (NOLOCK) WHERE DepoKodu=N'{0}'  ", label_SECILEN.Text);
				System.Data.DataTable dt=new System.Data.DataTable();
		...
		private void textBox_BARKOD_KeyPress(object sender, System.Windows.Forms.KeyPressEventArgs e)
		{
			if(e.KeyChar == 13)
			{
                Utility.Engine.sql = string.Format("SELECT DISTINCT   DepoKodu,Sevk   FROM  depo_tanim (NOLOCK) WHERE DepoKodu=N'{0}' and DepoKodu not in (select AD from haric_depo)   ", textBox_BARKOD.Text.ToUpper().Trim());
